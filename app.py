...
def load_data():
    ...
    # Aqui estamos simulando a criação de um dataframe em branco com a estrutura requerida 
    # caso os dados reais não existam para não quebrar a aplicação.
    try:
        df_semanal = pd.read_excel("dados.xlsx", sheet_name="Semanal")
        df_lives = pd.read_excel("dados.xlsx", sheet_name="Lives e Grupos")
    except:
        # DADOS SIMULADOS CASO NÃO ACHE O ARQUIVO (Apenas para fins de demonstração)
        df_semanal = pd.DataFrame({
            'Semana': [1, 2],
            'Investimento (R$)': [1000, 1500],
            'Leads Ads': [500, 800],
            'Leads Entrada': [450, 700],
            'Leads Saída': [50, 100],
            'Vendas Total': [10, 25],
            'Receita (R$)': [5000, 12500]
        })
        
        df_lives = pd.DataFrame({
            'Semana': [1, 1, 2],
            'Tipo': ['LVP', 'LVG', 'LVP'],
            'Label': ['LVP 1', 'LVG 1', 'LVP 1'],
            'Data': ['27/01', '30/01', '03/02'],
            'Cliques Total': [150, 200, 300],
            'Pico': [80, 120, 150],
            'Vendas': [5, 5, 25],
            'Grupo Ativo': ['GP1', 'GP1', 'GP2'],
            'Leads GP1': [450, 400, 380],
            'Cliques GP1': [150, 200, 50],
            'Leads GP2': [0, 0, 700],
            'Cliques GP2': [0, 0, 250],
        })
